...
export default function Home() {
  ...
  useEffect(() => {
    if(window!==undefined && localStorage.getItem("cartItems") === null)
  ...
  return (
    <div className="m-2 grid place-items-center gap-y-[20px] grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
      <Link href="/cart" className="fixed top-2 right-2 z-10 rounded-full border-[1px] bg-white hover:bg-gray-500 hover:cursor-pointer">
        <span className="absolute -top-2 -right-2 z-20 inline-flex h-5 w-5 rounded-full text-xs font-bold text-center">{window!==undefined ? JSON.parse(localStorage.getItem("cartItems") || "[]").length : 0}</span>
